...
async function main() {
    ...
    const authTx1 = await icebergExtension.setKeeperAuthorization(keeper.address, true);
    await authTx1.wait();
    const authTx2 = await icebergExtension.setKeeperAuthorization(await icebergKeeper.getAddress(), true);
    await authTx2.wait();
    ...
    const authTx3 = await icebergKeeper.setKeeperAuthorization(keeper.address, true);
    await authTx3.wait();
    ...
    const depositTx = await weth.connect(trader).deposit({ value: ether('100') }); // 100 WETH
    ...
    // Approve tokens
    const approveTx1 = await weth.connect(trader).approve(await swap.getAddress(), ether('100'));
    await approveTx1.wait();
    const approveTx2 = await dai.connect(taker1).approve(await swap.getAddress(), ether('500000'));
    await approveTx2.wait();
    const approveTx3 = await dai.connect(taker2).approve(await swap.getAddress(), ether('500000'));
    await approveTx3.wait();
    const approveTx4 = await inch.connect(taker1).approve(await swap.getAddress(), ether('100000'));
    await approveTx4.wait();
    
    console.log(`✅ Tokens minted and approved`);
    console.log(`   📝 Approval Txs: ${approveTx1.hash.slice(0,10)}... ${approveTx2.hash.slice(0,10)}... ${approveTx3.hash.slice(0,10)}... ${approveTx4.hash.slice(0,10)}...`);
    ...
    const configTx1 = await icebergExtension.connect(trader).configureIceberg(fixedSizeOrderHash, trader.address, {
    ...
    const fill1 = await executePartialFill(swap, fixedSizeOrder, fixedSizeSignature, ether('2'), taker1, "Fill #1");
    ...
    const revealTx1 = await icebergExtension.connect(keeper).revealNextChunk(fixedSizeOrderHash);
    const revealReceipt1 = await revealTx1.wait();
    console.log(`   📝 Reveal Tx Hash: ${revealTx1.hash}`);
    console.log(`   ⛽ Reveal Gas Used: ${revealReceipt1.gasUsed.toLocaleString()}`);
    await displayIcebergStatus(icebergExtension, fixedSizeOrderHash, "After Chunk Reveal");
    
    const fill2 = await executePartialFill(swap, fixedSizeOrder, fixedSizeSignature, ether('2'), taker1, "Fill #2");
    ...
    const configTx2 = await icebergExtension.connect(trader).configureIceberg(percentageOrderHash, trader.address, {
    ...
    const fill3 = await executePartialFill(swap, percentageOrder, percentageSignature, ether('5'), taker2, "Fill 10% of 50 WETH");
    
    await waitTime(61);
    const revealTx2 = await icebergExtension.connect(keeper).revealNextChunk(percentageOrderHash);
    ...
    const fill4 = await executePartialFill(swap, percentageOrder, percentageSignature, ether('4.5'), taker2, "Fill 10% of 45 WETH");
    ...
    const configTx3 = await icebergExtension.connect(trader).configureIceberg(adaptiveOrderHash, trader.address, {
    ...
    const fill5 = await executePartialFill(swap, adaptiveOrder, adaptiveSignature, ether('1.5'), taker1, "Quick Fill #1");
    await waitTime(30); // Fast fill
    
    const revealTx3 = await icebergExtension.connect(keeper).revealNextChunk(adaptiveOrderHash);
    ...
    const configTx4 = await icebergExtension.connect(trader).configureIceberg(timeBasedOrderHash, trader.address, {
    ...
    const fill6 = await executePartialFill(swap, timeBasedOrder, timeBasedSignature, ether('1'), taker1, "Initial 1 WETH chunk");
    
    // Wait multiple intervals to show growth
    console.log(`\n⏰ Waiting 3 minutes to show chunk growth...`);
    await waitTime(180);
    
    const revealTx4 = await icebergExtension.connect(keeper).revealNextChunk(timeBasedOrderHash);
    ...
    // Configure iceberg
    const configTx5 = await icebergExtension.connect(trader).configureIceberg(keeperOrderHash, trader.address, {
    ...
    if (needsUpkeep) {
        ...
        const upkeepTx = await icebergKeeper.connect(keeper).performUpkeep(performData);
        ...
        console.log(`   ⛽ Upkeep Gas Used: ${upkeepReceipt.gasUsed.toLocaleString()}`);
    }
    ...
    console.log(`   📝 Complete Transaction Tracking - All operations on-chain`);
    ...
    console.log(`   • Complete on-chain execution with verifiable transactions`);
    ...
    console.log(`   Average Gas per Tx: ${(totalGasUsed / BigInt(txHashes.length)).toLocaleString()}`);
    ...
    const orders = [
        { hash: fixedSizeOrderHash, name: "FIXED_SIZE Order" },
        { hash: percentageOrderHash, name: "PERCENTAGE Order" },
        { hash: adaptiveOrderHash, name: "ADAPTIVE Order" },
        { hash: timeBasedOrderHash, name: "TIME_BASED Order" },
        { hash: keeperOrderHash, name: "KEEPER Order" }
    ];
    
    for (const order of orders) {
        const config = await icebergExtension.icebergConfigs(order.hash);
        const progress = config.totalMakingAmount > 0 ? 
            ((Number(config.filledAmount) / Number(config.totalMakingAmount)) * 100).toFixed(1) : 0;
        console.log(`   ${order.name}: ${progress}% filled (${ethers.formatEther(config.filledAmount)}/${ethers.formatEther(config.totalMakingAmount)} WETH)`);
    }
